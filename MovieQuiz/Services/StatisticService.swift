import Foundation

final class StatisticService: StatisticServiceProtocol {
    private let storage: UserDefaults = .standard
    private enum Keys: String {
        case totalCorrect
        case bestGameCorrect
        case bestGameTotal
        case bestGameDate
        case totalGamesCount
    }
    var lastGameCorrect = 0
    
    var totalGamesCount: Int {
        get {
            storage.integer(forKey: Keys.totalGamesCount.rawValue)
        }
        set {
            storage.set(newValue, forKey: Keys.totalGamesCount.rawValue)
        }
    }
    var totalCorrect: Int {
        get {
            storage.integer(forKey: Keys.totalCorrect.rawValue)
        }
        set {
            storage.set(newValue, forKey: Keys.totalCorrect.rawValue)
        }
    }
    
    var bestGame: GameResult {
        get {
            let correct = storage.integer(forKey: Keys.bestGameCorrect.rawValue)
            let total = storage.integer(forKey: Keys.bestGameTotal.rawValue)
            let date: Date = storage.object(forKey: Keys.bestGameDate.rawValue) as? Date ?? Date()
            return GameResult(correct: correct,
                              total: total,
                              date: date)
        }
        
        set {
            storage.set(newValue.correct, forKey: Keys.bestGameCorrect.rawValue)
            storage.set(newValue.total, forKey: Keys.bestGameTotal.rawValue)
            storage.set(newValue.date, forKey: Keys.bestGameDate.rawValue)
        }
    }
    
    var totalAccuracy: Double {
        guard totalGamesCount > 0 else {
            return 0
        }
    return (Double(totalCorrect) / (Double(totalGamesCount) * 10)) * 100
    }
    
    func store(correct count: Int, total amount: Int) {
        totalGamesCount += 1
        totalCorrect += count
        lastGameCorrect = count
        if lastGameCorrect > bestGame.correct {
            bestGame = GameResult(correct: lastGameCorrect, total: totalGamesCount, date: Date())
        }
    }
    func reset() {
        for key in storage.dictionaryRepresentation().keys {
            storage.removeObject(forKey: key)
        }    }
    
}
